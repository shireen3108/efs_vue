  ...
  <v-main>
    ...
    <v-container fluid>
      <v-row align="center" justify="center">
                    <span class="headline">Stocks </span>
                      ...
                      <v-card>
                        ...
                        <v-list v-show="isExpanded(item)" dense>
                          ...
                          <v-list-item>
                            ...
                            <v-list-item-content class="align-end">{{ item.shares }} on {{ item.acquired_date }}</v-list-item-content>
                          </v-list-item>
                          <v-list-item>
                            <v-list-item-content>Recent value:</v-list-item-content>
                            <v-list-item-content class="align-end">{{ item.purchase_date }} on {{ item.recent_date }}</v-list-item-content>
                          </v-list-item>
                        </v-list>
                      </v-card>
                    </v-col>
    ...
    methods: {
      ...
      getStocks() {
        ...
        apiService.getStockList().then(response => {
          ...
          this.stocks = response.data.data.filter(a=>  {
             if (this.admins.includes(localStorage.getItem("log_user").replaceAll("\"","")))
             return true; 
            else if( a.cust_number == JSON.parse(localStorage.getItem("log_user")))
            return true;
          }
           );
      ...
      deleteStock(stock) {
        this.$confirm(`Are you sure, you want to delete the stock '${stock.name}(${stock.shares} shares)'?`).then(() => {
